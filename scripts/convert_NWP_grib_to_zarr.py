...
# * Fix this issue. Looks like it's running out of RAM.  Maybe the iterator is accumulating datasets?  Or maybe the processes are reading data more quickly than it's being written to disk, so we're accumulating data?  If so, maybe work in chunks:  Process a handful of grib files at a time.
# 
# ```
...
DST_ZARR_PATH = Path("/mnt/storage_ssd/data/ocf/solar_pv_nowcasting/nowcasting_dataset_pipeline/NWP/UK_Met_Office/UKV/zarr/test.zarr")
...
def decode_and_group_grib_filenames(filenames: list[Path], n_grib_files_per_nwp_init_time: int=2) -> pd.Series:
    ...
    map_datetime_to_filename = pd.Series(filenames, index=nwp_init_datetimes, name="nwp_grib_filenames")
...
def load_grib_files_from_groupby_tuple(input_tuple: tuple[pd.Timestamp, pd.Series]) -> xr.Dataset:
    full_grib_filenames = input_tuple[0]
    previous_lock = input_tuple[1]
    next_lock = input_tuple[2]
...
if DST_ZARR_PATH.exists():
    ...
    print(DST_ZARR_PATH, "exists.  The last NWP init datetime (UTC) in the Zarr is", last_nwp_init_datetime_in_zarr)
...
for _, full_grib_filenames in map_datetime_to_grib_filename.groupby(level=0):
    next_lock = multiprocessing.Lock()
    next_lock.acquire()
    queue.append((full_grib_filenames, previous_lock, next_lock))
